...
*   **SSO & Email Integration:** 🔐
    ...
    *   **Email-to-Task Mastery:**
        ...
        *   **Fault-Tolerant Polling & Catch-Up:** Implements a 30-second per-command IMAP timeout (`IMAP4_SSL(timeout=30.0)`) and automatically aborts the active batch poll if an IMAP connection or socket timeout (`CommandTimeout`) is encountered. The 7-day search window ensures Nimbus automatically recovers and backfills all missed emails following multi-day token expirations or system downtime.
        *   **Credential & Failure Observability:** Background token refresh failures, SSO login errors, IMAP authentication rejections, and connection drops are automatically logged to the `AuditLog` table with intelligent 1-hour rate limiting per user, providing immediate visibility in the UI without flooding audit tables.
