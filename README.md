...
*   **SSO & Email Integration:** 🔐
    ...
    *   **Email-to-Task Mastery:**
        *   **Automation:** Toggle automatic task generation in your User Settings. The background worker polls for emails from the last 3 days every 60 seconds and uses the default chat model (Gemma 3 4B via MLX) to extract structured tasks into your **General** project. The implementation uses lightweight header pre-fetching (to download only `Message-ID`, `Subject`, and `Date`) and matches them against database `AuditLog` records from the last 3 days to avoid double-processing already-read or already-processed emails. The full body is only fetched for new emails using atomic flagging (`BODY.PEEK`), preventing data loss during network or AI timeouts.
        *   **Fault-Tolerant Polling:** Implements a 30-second per-command IMAP timeout (`IMAP4_SSL(timeout=30.0)`) and automatically aborts the active batch poll if an IMAP connection or socket timeout (`CommandTimeout`) is encountered, preventing cascading delay cycles across remaining queued emails.
