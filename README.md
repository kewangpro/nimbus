...
*   **SSO & Email Integration:** 🔐
    ...
    *   **Email-to-Task Mastery:**
        *   **Automation:** Toggle automatic task generation in your User Settings. The background worker polls for emails from the last 7 days every 60 seconds and uses the default chat model (Gemma 3 4B via MLX) to extract structured tasks into your **General** project. The implementation uses lightweight header pre-fetching (to download only `Message-ID`, `Subject`, and `Date`) and matches them against database `AuditLog` records from the last 14 days to avoid double-processing already-read or already-processed emails. The full body is only fetched for new emails using atomic flagging (`BODY.PEEK`), preventing data loss during network or AI timeouts.
        *   **Fault-Tolerant Polling & Catch-Up:** Implements a 60-second per-command IMAP timeout (`IMAP4_SSL(timeout=60.0)`), immediate 3-second retry on connection drops, and automatic batch abort upon socket timeouts (`CommandTimeout`). The 7-day search window (`UNSEEN SINCE <7 days ago>`) ensures Nimbus automatically recovers and backfills all missed emails following multi-day token expirations or system downtime without slowing down on large mailboxes.
        *   **Transient vs. Permanent Error Classification & Auto-Retry:**
            *   **Transient Errors (`is_transient: true`):** Temporary network drops, IMAP connection timeouts, and provider 5xx HTTP gateway errors are classified as transient. The poller retries immediately once after a 3-second backoff, and the background worker retries automatically every 60 seconds. Emails encountering network drops remain `UNSEEN` in the mailbox and are automatically picked up on subsequent cycles.
            *   **Permanent Errors (`is_transient: false`):** Revoked OAuth tokens, missing refresh credentials, or expired client secrets are classified as permanent, requiring user SSO re-login. Corrupt payloads or unrecoverable database errors are flagged `\Seen` and logged to prevent infinite poison-pill retry loops.
        *   **Credential & Failure Observability:** Background token refresh failures, SSO login errors, IMAP authentication rejections, and connection drops are logged to `AuditLog` with 1-hour rate limiting per user. The Activity / Audit Logs modal visually flags transient retries (amber with `[Transient - Auto-Retrying]`) versus permanent failures (red with `[Permanent - Action Required]`). When the background worker successfully reconnects on a subsequent retry, an `Email Connection Restored` event (`email.connection_recovered`) is automatically recorded with a green indicator, giving clear visual confirmation of system self-healing.
        *   **SSL Handshake Resiliency & Fast Failure:** Enhanced IMAP connection tracking catches socket and TLS handshake exceptions (e.g., `SSLError: WRONG_VERSION_NUMBER` or gateway resets) immediately, avoiding 60-second blocking timeouts and eliminating unretrieved asyncio task exceptions.
        ...
        *   **Separation of Content & Summary:** Email-generated tasks store the complete raw email content in the task's **Description**, while saving the AI-extracted actionable insights directly into the **AI Summary** field (`IssueSummary`). Existing email tasks can be backfilled using `scripts/backfill_email_tasks.py` with flexible filtering (e.g. `--status todo --cutoff-time yesterday`).
        *   **HTML/CSS Sanitization:** Incoming emails and newsletters with complex HTML or corrupt plain-text stylesheets (`@media`, `@font-face`, etc.) are processed via `app.core.email_utils` to strip CSS/script blocks and extract clean, readable text descriptions.
        ...
        *   **Resilient Fallback:** If AI extraction fails or returns no tasks, Nimbus automatically creates a structured task with the full email body in **Description** and a concise notification in **AI Summary**, ensuring no important information is ever lost.
...
*   **Audit Logs:** Built-in auditing engine keeping track of workspace events (Project/Task CRUD), email automation, credential and connection failures, AI-driven scheduling, authentication, and file uploads. Captures entity titles, specific field-level changes, and detailed error descriptions for full operational visibility and accountability directly in the Activity / Audit Logs modal. ([View events list](docs/AUDITS.md))
...
- **Worker Timeout Safety:** Long-running periodic jobs like email polling are wrapped in a 120-second timeout to prevent the worker from hanging indefinitely on silent network connection drops. If a job times out, it is aborted and logged, freeing the worker to process subsequent queued tasks.
