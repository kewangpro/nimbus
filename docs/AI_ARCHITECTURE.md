...
*   **Timeout & Fault Tolerance:** Configured with a 60.0s command timeout (`IMAP4_SSL(timeout=60.0)`). Connection tasks are actively monitored to fast-fail TLS/TCP handshake errors (such as `SSLError: WRONG_VERSION_NUMBER`), preventing unretrieved asyncio task exceptions in uvloop and eliminating 60s blocking hangs. Transient connection drops trigger an immediate 3-second retry before reporting.
*   **Token Refresh & Error Observability:** Tokens are checked before every IMAP connection. If expired (within 5 minutes), they are refreshed automatically via the provider's token endpoint. If IMAP `AUTHENTICATE XOAUTH2` returns `NO` (common with a stale-but-unexpired Outlook access token), the poller invalidates the token expiry, refreshes, reconnects, and retries IMAP once in the same cycle. `email.auth_failed` is logged only if that retry still fails. Refresh failures, IMAP auth failures after retry, or connection failures are logged to `AuditLog` (throttled to at most 1 entry per hour per user). When a subsequent poll successfully connects and authenticates after `email.connection_failed` or `email.auth_failed`, an `email.connection_recovered` event is recorded.
*   **Transient vs. Permanent Error Classification:**
    *   **Transient Failures (`is_transient: true`):** IMAP connection timeouts, socket drops, SSL handshake errors, and provider 5xx HTTP gateway errors are classified as transient. The message remains `UNSEEN` in the mailbox and is excluded from `processed_ids`, guaranteeing automatic retry on the next 60-second polling cycle.
